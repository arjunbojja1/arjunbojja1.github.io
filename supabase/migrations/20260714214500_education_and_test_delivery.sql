...
add column education_level text check (
  education_level is null
  or education_level in ('bachelors', 'masters', 'phd')
);
...
create or replace function public.get_job_feed_v2(
...
as $$
  ...
  education_groups as (
    ...
    from public.jobs
    where jobs.education_level is not null
...
revoke all on function public.get_job_feed_v2(
  text,
  text,
  text[],
  public.job_source[],
...
grant execute on function public.get_job_feed_v2(
  text,
  text,
  text[],
  public.job_source[],
